 ...
 */
function withAffiliateLinks(printing: Printing, affiliateId: string | undefined): Printing {
...
function oracleCopyableText(oracle: Oracle): string {
  ...
  const typePart = [oracle.card_type, oracle.supertype].filter(Boolean).join(" — ");
...
export function cardsRoutes(cardProvider: CardDataProvider) {
  ...
  const preparePrinting = (printing: Printing, include: string | undefined): Printing => {
    ...
    new Elysia()
      ...
      .get(
        ...
        },
        {
          query: t.Object({
            include: t.Optional(
              t.String({ description: "Extra fields to include, e.g. `prices`" }),
            ),
          }),
          response: {
            200: OracleSchema,
      ...
      .get(
        ...
        },
        {
          query: t.Object({
            ...
            printing: t.Optional(
              t.String({
                description: "Printing id. Resolves to its oracle and marks that printing current.",
              }),
            ),
            slug: t.Optional(
              t.String({
      ...
      .get(
        ...
        },
        {
          ...
          response: {
            200: t.String({
              description: "Copy-pasteable plain-text card summary (text/plain; charset=utf-8)",
            }),
            404: ErrorSchema,
          },
          detail: {
      ...
      .get(
        ...
        async ({ params, query, set }) => {
          ...
          return oracleOut(viewedThrough(resolved.oracle, resolved.printing), query.include);
        },
        {
      ...
      .get(
        ...
        async ({ query, set }) => {
          ...
          const offsetTooLarge = Number.isFinite(parsedOffset) && parsedOffset > MAX_SEARCH_OFFSET;
          ...
          const pageLimit = Number.isFinite(parsedLimit) && parsedLimit > 0 ? parsedLimit : 10;
        ...
        },
        {
          query: t.Object({
            ...
            type: t.Optional(
              t.String({
                description:
                  "Optional explicit type filter, merged as `AND t:value` with the parsed query.",
              }),
            ),
            artist: t.Optional(
              t.String({
            ...
            rarity: t.Optional(
              t.String({
                description:
                  "Optional explicit rarity filter, merged as `AND r:value`. Rarity is printing-level.",
              }),
            ),
            ...
            fuzzy: t.Optional(
              t.String({
                description:
                  "Pass `false` or `0` to disable fuzzy/autocomplete matching (exact name only).",
              }),
            ),
            browse: t.Optional(
              t.String({ description: "Pass `all` to browse every card without a search query." }),
            ),
            unique: t.Optional(
              t.String({
                description:
            ...
            limit: t.Optional(
              t.String({ description: "Max results per page (default 10, max 100)" }),
            ),
            offset: t.Optional(
          ...
          response: {
            200: t.Object({
              ...
              total: t.Optional(t.Number({ description: "Total matching rows for this query" })),
        ...
        async ({ body, set }) => {
          ...
          return {
            ...
            results: results.map((result) => ({
              ...result,
              oracle: result.oracle ? oracleOut(result.oracle, body.include) : null,
              printing: result.printing ? printingOut(result.printing, body.include) : null,
            })),
          };
        },
        {
