...
const tokenResolver = async (token: string) => {
  if (token === "admin-token")
    return { id: ADMIN_ID, email: "admin@example.com", created_at: "2026-08-01" };
  if (token === "user-token")
    return { id: USER_ID, email: "user@example.com", created_at: "2026-08-01" };
  return null;
};
...
class StubRepository implements AdminDataRepository {
  ...
  async getOracleName(id: string) {
    return id === ORACLE_ID ? "Test Card" : null;
  }
  async getPrintingOracleId(id: string) {
    return id === PRINTING_ID ? ORACLE_ID : null;
  }
  async getTakenPrintingSlugs() {
    return this.takenPrintingSlugs;
  }
  async getTakenOracleSlugs() {
    return this.takenOracleSlugs;
  }
      ...
      printings: [
        {
          ...
          source: "riftcodex",
          deleted_at: null,
          locked_fields: ["rarity"],
          oracle_locked_fields: ["might"],
          delta_source: "admin" as const,
  ...
  async listReconciliation(query: unknown) {
    ...
    return {
      entries: this.entry ? [this.entry] : [],
      total: this.entry ? 1 : 0,
      counts: { pending: 1, confirmed: 0, dismissed: 0 },
    };
  }
  async getReconciliationEntry() {
    return this.entry;
  }
...
describe("admin API contracts", () => {
  ...
  function makeApp() {
    return new Elysia({ prefix: "/api/v1" }).use(
      adminRoutes({ repository, imageBindings: images, adminAuthPlugin: adminPlugin }),
    );
  }
  beforeEach(() => {
  ...
  test("auth gate rejects missing and non-admin tokens before any repository call", async () => {
    ...
    expect(
      (await app.handle(request(path, "PATCH", { patch: { power: 1 } }, "user-token"))).status,
    ).toBe(403);
  ...
  test("audit log forwards a bounded, trimmed query", async () => {
    const response = await app.handle(
      request("/admin/audit-log?limit=999&offset=-2&action=%20oracle.patch%20&target_type=oracle"),
    );
    expect(response.status).toBe(200);
  ...
  test("printing list defaults to live, bounds the page and upper-cases the set", async () => {
    const response = await app.handle(
      request("/admin/printings?limit=999&offset=-5&set=%20ogn%20&q=%20vex%20"),
    );
    expect(response.status).toBe(200);
  ...
  test("reconciliation list defaults to pending and preserves source/kind filters", async () => {
    const response = await app.handle(
      request("/admin/reconciliation?kind=field_diff&source=gallery&limit=999"),
    );
    expect(response.status).toBe(200);
  ...
  test("confirming an unmatched product writes a locked printing patch", async () => {
    const response = await app.handle(
      request(`/admin/reconciliation/${ENTRY_ID}/confirm`, "POST", { note: "matched" }),
    );
    expect(response.status).toBe(200);
    expect(repository.calls).toEqual([
      {
        ...
        args: {
          ...
          p_patch: {
            tcgplayer_id: "652952",
            tcgplayer_url: "https://www.tcgplayer.com/product/652952/test",
          },
          p_note: "matched",
          p_actor: ADMIN_ID,
        },
  ...
  test("confirming an oracle field patches the oracle before closing the entry", async () => {
    repository.entry = {
      ...productEntry(),
      kind: "field_diff",
      payload: { field: "energy", current_value: "2", proposed_value: "3", oracle_id: ORACLE_ID },
    ...
    expect(response.status).toBe(200);
    expect(repository.calls.map((call) => call.name)).toEqual([
      "admin_patch_oracle",
  ...
  test("unsupported reconciliation fields are rejected without a write", async () => {
    repository.entry = {
      ...productEntry(),
      kind: "field_diff",
      payload: { field: "text", proposed_value: "changed" },
    };
    const response = await app.handle(
      request(`/admin/reconciliation/${ENTRY_ID}/confirm`, "POST", {}),
    );
  ...
  test("dismissing a reconciliation entry never carries a card patch", async () => {
    const response = await app.handle(
      request(`/admin/reconciliation/${ENTRY_ID}/dismiss`, "POST", { note: "expected" }),
    );
    expect(response.status).toBe(200);
  ...
  test("creating an oracle normalizes its keys and collision-suffixes its slug", async () => {
    ...
    const response = await app.handle(
      request("/admin/oracles", "POST", { definition: { name: "  Test-Card  ", might_bonus: 0 } }),
    );
    expect(response.status).toBe(200);
    expect(repository.calls[0]).toEqual({
      name: "admin_create_oracle",
      args: expect.objectContaining({
        p_oracle_key: "test card",
        p_slug: "test-card-2",
  ...
  test("patching an oracle derives name fields and preserves might_bonus: 0", async () => {
    const response = await app.handle(
      request(`/admin/oracles/${ORACLE_ID}`, "PATCH", {
        patch: { name: "  New-Name ", might_bonus: 0 },
      }),
    );
    expect(response.status).toBe(200);
    expect(repository.calls[0]).toEqual({
      name: "admin_patch_oracle",
      args: {
  ...
  test("oracle delete and restore call their soft-delete RPCs", async () => {
    expect(
      (await app.handle(request(`/admin/oracles/${ORACLE_ID}`, "DELETE", { reason: "duplicate" })))
        .status,
    ).toBe(200);
    expect((await app.handle(request(`/admin/oracles/${ORACLE_ID}/restore`, "POST"))).status).toBe(
      200,
    );
    expect(repository.calls.map((call) => call.name)).toEqual([
      "admin_delete_oracle",
      "admin_restore_oracle",
    ]);
  });
  ...
  test("oracle relationships expose reverse edges and replace only unique non-self edges", async () => {
    ...
    expect(
      (await app.handle(request(`/admin/oracles/${ORACLE_ID}/relationships`, "PUT", body))).status,
    ).toBe(200);
    expect(repository.calls[0]?.name).toBe("admin_set_oracle_relationships");
    expect(
      (
        await app.handle(
          request(`/admin/oracles/${ORACLE_ID}/relationships`, "PUT", {
            entries: [{ kind: "signature", to_oracle_id: ORACLE_ID }],
          }),
        )
      ).status,
    ).toBe(400);
  ...
  test("creating a printing generates and pins a collision-safe slug", async () => {
    ...
    const response = await app.handle(
      request("/admin/printings", "POST", {
        id: PRINTING_ID,
        oracle_id: ORACLE_ID,
        set_code: "ogn",
        definition: { collector_number: "12", rarity: "Rare" },
      }),
    );
    expect(response.status).toBe(200);
    expect(repository.calls[0]).toEqual({
      name: "admin_create_printing",
      args: expect.objectContaining({
        p_printing_id: PRINTING_ID,
        p_oracle_id: ORACLE_ID,
        p_set_code: "OGN",
        p_public_slug: "ogn/12/test-card-2",
        p_actor: ADMIN_ID,
      }),
    });
  });
  ...
  test("patching a printing uppercases a moved set and keeps rarity printing-level", async () => {
    const response = await app.handle(
      request(`/admin/printings/${PRINTING_ID}`, "PATCH", {
        patch: { set_code: "ven", rarity: "Showcase" },
      }),
    );
    expect(response.status).toBe(200);
  ...
  test("printing delete and restore call their scoped RPCs", async () => {
    ...
    expect(repository.calls.map((call) => call.name)).toEqual([
      "admin_delete_printing",
      "admin_restore_printing",
    ]);
  });
  ...
  test("regenerating a printing slug uses resolved name fields and collision suffixing", async () => {
    repository.takenPrintingSlugs.add("ogn/12/test-card");
    const response = await app.handle(
      request(`/admin/printings/${PRINTING_ID}/regenerate-slug`, "POST"),
    );
    expect(response.status).toBe(200);
  ...
  test("delta read, write, and clear share the admin delta contract", async () => {
    expect(
      ((await (await app.handle(request(`/admin/printings/${PRINTING_ID}/deltas`))).json()) as any)
        .delta.tags_added,
    ).toEqual(["Sentinel"]);
    await app.handle(
      request(`/admin/printings/${PRINTING_ID}/deltas`, "PUT", {
        delta: { tags_added: ["Scout"], cleared_fields: ["energy"] },
      }),
    );
    await app.handle(request(`/admin/printings/${PRINTING_ID}/deltas`, "PUT", { delta: null }));
    expect(repository.calls.map((call) => call.args.p_delta)).toEqual([
      { tags_added: ["Scout"], cleared_fields: ["energy"] },
      null,
    ]);
  });
  ...
  test("image upload stores a content-addressed source, persists it, and queues variants", async () => {
    ...
    const response = await app.handle(
      new Request(`http://localhost/api/v1/admin/printings/${PRINTING_ID}/image`, {
        method: "POST",
  ...
  test("printing legalities expose resolved precedence", async () => {
    ...
    expect(response.status).toBe(200);
    expect(((await response.json()) as any).entries[0]).toMatchObject({
      status: "banned",
      scope: "oracle",
    });
  });
  ...
  test("legality writes choose printing or oracle scope and null clears to default", async () => {
    await app.handle(
      request(`/admin/printings/${PRINTING_ID}/legalities`, "PUT", {
        format_code: "STANDARD",
        status: "default",
      }),
    );
    ...
    expect(repository.calls[1]?.args).toMatchObject({
      p_oracle_id: ORACLE_ID,
      p_printing_id: null,
      p_status: "banned",
    });
  ...
  test("format zone rules keep null bounds as unconstrained rather than zero", async () => {
    ...
    expect(listed.formats[0].zone_rules[0]).toEqual({
      zone: "sideboard",
      min_count: null,
      max_count: 10,
      copy_limit: 3,
    });
    expect(listed.formats[0].severity_overrides[0]).toEqual({
      status: "restricted",
      severity: "warning",
    });

    const saved = await app.handle(
      request("/admin/formats/STANDARD/zone-rules/main", "PUT", { max_count: 40, copy_limit: 3 }),
    );
  ...
  test("an unknown zone never reaches the database", async () => {
    const response = await app.handle(
      request("/admin/formats/standard/zone-rules/graveyard", "PUT", { max_count: 1 }),
    );
    expect(response.status).toBe(400);
  ...
  test("severity default clears the override instead of storing one", async () => {
    await app.handle(
      request("/admin/formats/STANDARD/severities/restricted", "PUT", { severity: "error" }),
    );
    await app.handle(
      request("/admin/formats/standard/severities/banned", "PUT", { severity: "default" }),
    );
    expect(repository.calls[0]?.args).toMatchObject({
      p_code: "standard",
      p_status: "restricted",
  ...
  test("rejected format rules map their reason to a status the UI can render", async () => {
    ...
    const range = await app.handle(
      request("/admin/formats/standard/zone-rules/main", "PUT", { min_count: 9, max_count: 4 }),
    );
    ...
    const missing = await app.handle(
      request("/admin/formats/nope/severities/legal", "PUT", { severity: "warning" }),
    );
  ...
  test("formats list and create include cascade counts and normalize the code", async () => {
    expect(
      ((await (await app.handle(request("/admin/formats"))).json()) as any).formats[0]
        .legality_count,
    ).toBe(2);
    await app.handle(
      request("/admin/formats", "POST", { code: "Standard_2", name: " Standard 2 " }),
    );
    expect(repository.calls[0]).toEqual({
  ...
  test("format reorder, patch, and delete each preserve their RPC contract", async () => {
    ...
    await app.handle(
      request("/admin/formats/STANDARD", "PATCH", { patch: { name: " Standard " } }),
    );
  ...
  test("ruling list does not invent a target-kind filter", async () => {
    ...
    expect(repository.rulingsQueries[0]).toEqual({
      limit: 50,
      offset: 0,
      query: "resolve",
      kind: undefined,
  ...
  test("ruling preview parses the shared search grammar without writing", async () => {
    const response = await app.handle(
      request("/admin/rulings/preview", "POST", { query: "t:unit", limit: 7 }),
    );
    expect(response.status).toBe(200);
    expect(repository.previewed[0]).toMatchObject({
      ast: { op: "filter", field: "type", value: "unit" },
      limit: 7,
    });
    expect(repository.calls).toEqual([]);
  });
  ...
  test("ruling creation stores direct targets and a parsed query AST", async () => {
    const response = await app.handle(
      request("/admin/rulings", "POST", {
        type: "ruling",
        text: " Rule text ",
        targets: [
          { kind: "oracle", oracle_id: ORACLE_ID },
          { kind: "query", query: "r:showcase" },
        ],
      }),
    );
    expect(response.status).toBe(200);
    ...
    expect(repository.calls[0]?.args.p_targets).toEqual([
      { kind: "oracle", oracle_id: ORACLE_ID },
      {
        kind: "query",
        query: "r:showcase",
        ast: { op: "filter", field: "rarity", value: "showcase" },
      },
    ]);
  });
  ...
  test("ruling patch replaces parsed targets and delete removes the ruling", async () => {
    await app.handle(
      request(`/admin/rulings/${RULING_ID}`, "PATCH", {
        patch: { targets: [{ kind: "printing", printing_id: PRINTING_ID }] },
      }),
    );
  ...
  test("set create, patch, and delete normalize codes and preserve lock-bearing payloads", async () => {
    await app.handle(
      request("/admin/sets", "POST", {
        set_code: "tst",
        definition: { set_name: " Test Set ", parent_set_code: "ogn" },
      }),
    );
    await app.handle(
      request("/admin/sets/tst", "PATCH", {
        patch: { set_name: " Renamed ", parent_set_code: "ven" },
      }),
    );
    ...
    expect(repository.calls[0]?.args).toMatchObject({
      p_set_code: "TST",
      p_definition: { set_name: "Test Set", parent_set_code: "OGN" },
    });
    expect(repository.calls[1]?.args.p_patch).toEqual({
      set_name: "Renamed",
      parent_set_code: "VEN",
    });
  });
  ...
  test("repository exceptions never leak database messages", async () => {
    ...
    const response = await app.handle(
      request(`/admin/oracles/${ORACLE_ID}`, "PATCH", { patch: { power: 2 } }),
    );
