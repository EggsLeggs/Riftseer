...
The API describes itself with Elysia schema annotations (`detail`, `query`, `body`, `response`) on each route. The OpenAPI spec is generated from those annotations, committed at `apps/api/openapi.json`, and served by the Worker itself.
...
`apps/api/src/app.ts` exports `buildApp(provider, options)`, the complete route composition. The Worker calls it with the Cloudflare adapter; `apps/api/scripts/generate-spec.ts` calls it under plain Bun with the in-memory stub provider and mounts `@elysiajs/swagger` beside it to extract the spec. Because both go through the same function, a mounted route is in the spec by construction — there is no second list to forget a route in.
...
```bash
cd apps/api
bun run generate:spec     # rewrites apps/api/openapi.json
```
...
The Discord bot (`apps/discord-bot/src/api.ts`) and the website consume it:
