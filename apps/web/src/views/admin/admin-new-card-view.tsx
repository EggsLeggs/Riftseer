...
export function AdminNewCardView() {
  ...
  const update = <K extends keyof GalleryPrefill>(key: K, value: GalleryPrefill[K]) =>
    setValues((current) => ({ ...current, [key]: value }));

  async function submit(event: React.FormEvent) {
    event.preventDefault();
    if (!isValidCardId(printingId))
      return void toast.error("Printing ID must be 24 hexadecimal characters");
    ...
    try {
      ...
      await printingMutations.create.mutateAsync([
        printingId,
        oracle.oracle_id,
        values.setCode,
        printingDefinition,
      ...
      if (reviewEntryId) {
        try {
          await reviewMutations.confirm.mutateAsync([reviewEntryId, printingId, oracle.oracle_id]);
        } catch {
  ...
  return (
    <>
      <AdminPageHeader
        title={reviewEntryId ? "Create missing card" : "New card"}
        description="Creates an oracle first, then one physical printing with a pinned public slug."
        crumbs={[
      ...
      <form onSubmit={(event) => void submit(event)} className="space-y-8">
        <AdminSection heading="Printing identity">
          <div className="flex flex-wrap items-end gap-3">
            <div className="min-w-72 flex-1">
              <Label htmlFor="new-printing-id">Printing ID</Label>
              <Input
                id="new-printing-id"
                value={printingId}
            ...
            <Button type="button" variant="outline" onClick={() => setPrintingId(generateCardId())}>
              <RefreshCw />
              Regenerate
            </Button>
          </div>
        ...
        <AdminSection heading="Oracle">
          <FieldGrid>
            ...
            <TextField
              ...
              label="Might"
              value={values.might}
              onChange={(event) => update("might", event.target.value)}
            />
            <TextField
              id="new-might-bonus"
              label="Might bonus"
              hint="0 is meaningful; blank means not equipment"
              value={values.mightBonus}
              onChange={(event) => update("mightBonus", event.target.value)}
            />
          </FieldGrid>
          <div className="mt-4">
            <CheckboxField
              id="new-token"
              label="Token"
              checked={values.isToken}
              onChange={(event) => update("isToken", event.target.checked)}
            />
          </div>
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div>
              <Label htmlFor="new-text">Rules text</Label>
              <Textarea
                id="new-text"
                rows={5}
                value={values.text}
            ...
            <div>
              <Label htmlFor="new-equipment">Equipment text</Label>
              <Textarea
                id="new-equipment"
                rows={5}
        ...
        <AdminSection heading="Printing">
          <FieldGrid>
            ...
            <TextField
              ...
              onChange={(event) => update("collectorNumber", event.target.value)}
            />
            <TextField
              id="new-rarity"
              label="Rarity"
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <CheckboxField
              ...
              onChange={(event) => update("signature", event.target.checked)}
            />
            <CheckboxField
              id="new-alt"
              label="Alternate art"
              checked={values.alternateArt}
              onChange={(event) => update("alternateArt", event.target.checked)}
            />
            <CheckboxField
              id="new-special"
              label="Special collection"
              checked={values.specialCollection}
              onChange={(event) => update("specialCollection", event.target.checked)}
            />
          </div>
          {values.imageUrl ? (
            <div className="mt-4 flex items-start gap-4">
              <img src={values.imageUrl} alt="" className="h-40 rounded border" />
              <CheckboxField
                id="new-upload"
                label="Upload gallery art"
                checked={uploadArt}
                onChange={(event) => setUploadArt(event.target.checked)}
              />
            </div>
          ) : null}
        </AdminSection>
        <Button type="submit" disabled={submitting}>
          {submitting ? "Creating…" : "Create oracle and printing"}
        </Button>
      </form>
    </>
  );
