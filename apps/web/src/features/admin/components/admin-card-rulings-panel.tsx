...
export function AdminCardRulingsPanel({ printing }: { printing: Printing }) {
  ...
  return (
    <AdminSection
      ...
      description="Rulings can target several cards or a saved query, so creation and retargeting live in the rulings manager."
    >
      <Button variant="outline" size="sm" asChild>
        <Link href="/admin/rulings">Manage rulings</Link>
      </Button>
      {rulings.isError ? (
        <p className="text-destructive mt-4 text-sm">Couldn&apos;t load rulings.</p>
      ) : rulings.isPending ? (
        ...
        <p className="text-muted-foreground mt-4 text-sm">No rulings reach this printing.</p>
      ) : (
          ...
          {entries.map((ruling) => (
            <li key={ruling.id} className="rounded-lg border p-4">
              <div className="mb-1 flex flex-wrap gap-2 text-xs text-muted-foreground">
                <span className="capitalize">{ruling.type}</span>
                <span className="capitalize">{ruling.scope}</span>
                {ruling.shared ? <span>{ruling.target_count} targets</span> : null}
                {ruling.dated ? <span>{ruling.dated}</span> : null}
              </div>
