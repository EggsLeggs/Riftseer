...
For local development with `wrangler dev`, set these in `apps/ingest-worker/.dev.vars` (never commit this file). For production deployment, store them with `wrangler secret put SUPABASE_URL` and `wrangler secret put SUPABASE_SERVICE_ROLE_KEY`.
