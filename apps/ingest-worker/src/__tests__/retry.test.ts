...
describe("ingest RPC retry", () => {
  test("classifies transient transport/gateway failures without matching incidental numbers", () => {
    ...
    for (const message of [
      "invalid id 68e5039a5031b1b0f1502503",
      "unknown collector ogn-503a",
      "duplicate key",
  ...
  test("returns immediately on success or a deterministic error", async () => {
    ...
    expect(
      await callRpcWithRetry<{ upserted: number }>(
        success.supabase as never,
        "ingest_catalogue",
        {},
        "upsert",
        { baseDelayMs: 0 },
    ...
    await expect(
      callRpcWithRetry(failure.supabase as never, "ingest_catalogue", {}, "prune", {
        baseDelayMs: 0,
      }),
  ...
  test("retries a transient failure until the eventual result", async () => {
    ...
    expect(
      await callRpcWithRetry<{ ok: boolean }>(
        scripted.supabase as never,
        "ingest_catalogue",
        { batch: 2 },
        "batch",
        { baseDelayMs: 0 },
  ...
  test("uses bounded exponential backoff and names the exhausted operation", async () => {
    const scripted = client(
      Array.from({ length: INGEST_RPC_MAX_ATTEMPTS }, () => ({
        error: { message: "internal error; reference = x" },
      })),
    );
    const delays: number[] = [];
    ...
    try {
      await expect(
        callRpcWithRetry(scripted.supabase as never, "ingest_catalogue", {}, "final prune"),
      ).rejects.toThrow("final prune failed");
    } finally {
