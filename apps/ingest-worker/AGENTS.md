...
- `POST /images/reconcile` re-sends the catalogue scan on its own. Step 12 is the last thing an ingest does, so a run that dies in step 9 takes image hosting with it and nothing revives it until a whole ingest goes green. `GET /images/status` reports how many printings are still unhosted; that number staying flat across runs is the only symptom a stall has.
...
curl http://localhost:8787/images/status         # printings still serving upstream art
