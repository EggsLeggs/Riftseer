...
- A failed scheduled run throws. `runIngest` catches everything and returns a result, which suits `POST /ingest`, but the cron handler used to log that result inside `waitUntil`, so Cloudflare recorded every run as a success while each one died at batch 7 of 9 for five weeks (#124). The handler now awaits the run and throws on `ok: false`, and the failure shows in the Cron Trigger past events. Cron triggers are not retried, so throwing changes what is reported, not what runs.
