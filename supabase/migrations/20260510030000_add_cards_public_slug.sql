-- Adds the human-friendly URL path stored on each card row.
--
-- public_slug is the relative site path (no leading slash) used by the Next.js
-- frontend, e.g. "ogn/12a/signature/sun-disc". It is generated by the ingest
-- worker on first insert and never overwritten on subsequent ingests so card
-- URLs stay stable as upstream data is corrected.

ALTER TABLE cards
  ADD COLUMN IF NOT EXISTS public_slug text;

CREATE UNIQUE INDEX IF NOT EXISTS cards_public_slug_uidx
  ON cards (public_slug)
  WHERE public_slug IS NOT NULL;

-- Re-create ingest_card_data to insert public_slug for new rows but never
-- update it on conflict — so URLs do not drift when names/collectors change.
