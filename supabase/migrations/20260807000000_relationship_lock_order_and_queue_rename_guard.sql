-- ── Relationship lock order, redundant index, queue rename guard ──────────────
-- Three corrections to the two migrations before this one. Each is written to
-- be safe on a database that already has the fixed state.
--
-- 1. `admin_set_card_relationships` locked the oracle group with an unordered
--    `SELECT ... FOR UPDATE`, so two concurrent saves in the same group could
--    take the same rows in opposite orders and deadlock. Restated here with
--    `ORDER BY c.id`; supersedes the body in
--    20260806000000_relationship_override_scopes.sql and is otherwise identical.
--
-- 2. `card_relationship_overrides_oracle_key_idx` is redundant:
--    `card_relationship_overrides_oracle_uidx` leads on `oracle_key` under the
--    same partial predicate and already serves every lookup by it.
--
-- 3. `reconciliation_queue.tcgplayer_payload → payload` in
--    20260805000000_phase8_special_collection_and_gallery_review.sql is an
--    unguarded `RENAME COLUMN` (which has no `IF EXISTS`). Repeated here as a
--    conditional, so a database left part-way through that push converges
--    instead of failing every attempt at the rename.

-- ── 1. Redundant index ────────────────────────────────────────────────────────

DROP INDEX IF EXISTS card_relationship_overrides_oracle_key_idx;

-- ── 2. Queue column rename, made re-runnable ──────────────────────────────────
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'tcgplayer_payload'
  ) AND NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'reconciliation_queue'
...
-- ── 3. admin_set_card_relationships ───────────────────────────────────────────
-- Unchanged from 20260806000000 except for the `ORDER BY c.id` on the oracle
-- group lock. The signature is identical, so `CREATE OR REPLACE` keeps the
-- existing privileges; the grants are restated below for a fresh database.
