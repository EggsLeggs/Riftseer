-- Add the related_signatures link column to cards.
--
-- related_signatures: populated on Legend and Champion cards — a RelatedCard[]
-- of the signature cards (classification.supertype = 'Signature', e.g. "Daisy!"
-- for Ivern) tied to them by a shared character tag. The reverse links live on
-- the signature card's related_legends / related_champions.
ALTER TABLE cards
  ADD COLUMN IF NOT EXISTS related_signatures jsonb NOT NULL DEFAULT '[]';

-- Re-create ingest_card_data to persist related_signatures. Identical to the
-- 20260510030000 definition except for the new column in the cards insert /
-- on-conflict update. public_slug is still never overwritten once set.
...
BEGIN
  ...
  LOOP
    INSERT INTO cards (
      ...
      is_token, public_slug, ingested_at
    )
    VALUES (
      c->>'id',
      c->>'name',
    ...
    ON CONFLICT (id) DO UPDATE SET
      -- public_slug stays put once set so card URLs do not drift between
      -- ingest runs. coalesce lets us backfill rows that were inserted before
      -- the slug column existed (or any other null slug) without overwriting
      -- existing values.
