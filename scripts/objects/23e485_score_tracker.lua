...
mod_name,version='ScoreTracker',1
...
local lCheck={
  ...
  ['double_my_score_']=function(n,c)
    if c==owner and activePlayerList[owner] then
      return count*2^n,'doubled their score this many times:',nil
    end
  end,
  ['reset_score_']=function(n,c)
    if c==owner then
      return n,'reset score to',nil
    else
      return n,false,nil
    end
  end,
  ['set_score_']=function(n,c)
    if c==owner then
      return n,'score changed by '..math.abs(n-count)..'. Setting it to',nil
    end
  end,
    ...
    for k,f in pairs(lCheck) do
      if m:find(k..'%-?%d+') then
        ...
        if not(activePlayerList[owner]) then
          ...
          if not(msg:lower():match('set score')) then
            return
          end
        end
...
function onload(s)
  ...
  ref_type = 'score'--self.getName():gsub('%s.+','')
  ...
  for i,v in ipairs({             -- the side buttons
    ...
    self.setVar(fn, function(o,c,a) local b=1 if a then b=2 end click_changeValue(o,c,v.n*b) end)
    self.createButton({
      tooltip='Right-click for '..v.n*2,
      label=v.l,
      position=v.p,
      click_function=fn,
      function_owner=self,
	...
	self.createButton({            -- reset score to 0 button
		label='[R]',
		click_function='resetScore',
		tooltip='Reset Score',
		function_owner=self,
...
function resetScore(obj,color)
  local prevCount=count
	sL(0,0)
	count=0
  onMightCountChanged(prevCount, count)
	printToAll(owner..'[999999] reset their score to [-]|'..count..'|',ownerRGB)
	updateSave()
end
...
function onNumberTyped(col,int)
  ...
  tID=Wait.time(function()
    if int~=count then
      ...
      printToAll(owner..'[999999]'..tx..math.abs(n)..' score [-]|'..count..'|',ownerRGB)
      updateSave()
    end
  end, 3)
end
