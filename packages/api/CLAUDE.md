...
- `GET /api/v1/admin/audit-log` is the only read endpoint under `/admin` — it pages the `admin_audit_log` table newest-first via `AdminDataRepository.listAuditLog`. The table is append-only; there is no write endpoint for it.
