...
- Read endpoints under `/admin`: `audit-log` (pages the `admin_audit_log` table newest-first via `AdminDataRepository.listAuditLog`; append-only, no write endpoint), `formats`, and the per-card `cards/:id/legalities` / `cards/:id/rulings`. The last two expose the card-level and per-printing layers separately, which the public payload does not — the editor needs to tell an inherited status from a printing-specific one.
