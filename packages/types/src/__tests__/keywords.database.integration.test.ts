...
async function sqlJson<T>(statement: string): Promise<T> {
  const subprocess = Bun.spawn([process.execPath, DATABASE_SCRIPT, "query", statement], {
    env: process.env,
    stdout: "pipe",
    stderr: "pipe",
  });
...
integration("card keyword TypeScript/Postgres conformance", () => {
  test("both derivations return the same sorted, distinct base keys", async () => {
    const inputs = JSON.stringify(KEYWORD_CONFORMANCE_CASES.map(({ input }) => input)).replaceAll(
      "'",
      "''",
    );
    ...
    const fromTypeScript = KEYWORD_CONFORMANCE_CASES.map(({ input }) => extractCardKeywords(input));
    expect(fromTypeScript).toEqual(KEYWORD_CONFORMANCE_CASES.map(({ expected }) => expected));
