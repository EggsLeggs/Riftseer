...
import type { DeckTextCard } from "../deck-text.ts";

import type { AddableCard } from "./add.ts";
import type { DeckCardChange } from "./changes.ts";
 ...
 * requirement.
 *
 * Everything here is pure; the localStorage calls are web's, because the
 ...
 *
 * The wire row the builder components already take, with `zone` narrowed to
 * the real vocabulary. Restated rather than derived from the API because this
 * is the shape on disk, which must not move when the wire does; web asserts
 * the two still agree. Storing the whole row rather than just the rules fields
 * is deliberate: the display fields (`set_code`,
