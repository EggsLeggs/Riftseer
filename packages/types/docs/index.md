...
`@riftseer/core` carries heavy server-side dependencies (Supabase client, etc.). Any package that only needs _types_ or the parser — the Discord bot, the ingest worker, external tooling — can import `@riftseer/types` directly without pulling in those transitive dependencies.
