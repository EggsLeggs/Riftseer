...
All canonical types live in `src/types.ts`. They mirror the Postgres schema in `supabase/migrations/` and are the single source of truth for the card shape across the whole monorepo.
...
Referenced inside `all_parts` and `used_by`:
...
See [Deck](./deck.md) for the richer in-memory `Deck` class and [Serialiser](./serialiser.md) for how `SimplifiedDeck` is encoded for URL sharing.

---

## Adding a field

If a new field is added to `Card`:

1. Update `src/types.ts`
2. Update `packages/ingest-worker/src/riftcodex.ts` (`rawToCard`)
3. Update the row mapping in `src/providers/supabase.ts` (`dbRowToCard`)
