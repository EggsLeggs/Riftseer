...
export function AdminCardRelationshipsPanel({ oracle }: { oracle: Oracle }) {
  ...
  function add(target: Oracle) {
    if (
      target.id === oracle.id ||
      entries.some((entry) => entry.kind === kind && entry.to_oracle_id === target.id)
    )
      return;
  ...
  return (
    <AdminSection
      heading="Oracle relationships"
      description="Relationships are oracle-scoped. Incoming edges are shown for context; saving replaces only this oracle's outgoing edges."
    >
      {relationships.isError ? (
        <p className="text-destructive text-sm">Couldn&apos;t load relationships.</p>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-[12rem_1fr]">
            <select
              ...
              value={kind}
              onChange={(event) => setKind(event.target.value as AdminRelationshipKind)}
            >
            ...
            <div className="relative">
              ...
              {results.data?.cards.length ? (
                <div className="bg-popover absolute z-20 mt-1 w-full rounded-md border p-1 shadow-md">
                  {results.data.cards.map(({ oracle: target }) => (
                    <button
                      key={target.id}
                      type="button"
          ...
          <ul className="mt-4 space-y-2">
            {entries.map((entry) => (
              <li
                key={`${entry.kind}:${entry.to_oracle_id}`}
                className="flex items-center justify-between rounded-md border px-3 py-2 text-sm"
              >
                <span>
                  <span className="text-muted-foreground mr-2">{KIND_LABELS[entry.kind]}</span>
                  {nameFor(entry.to_oracle_id)}
                </span>
                <Button
                  type="button"
                  variant="ghost"
          ...
          {relationships.data?.incoming.length ? (
            <div className="mt-5">
              <h3 className="mb-2 text-sm font-medium">Incoming (read-only)</h3>
              <ul className="space-y-1 text-sm text-muted-foreground">
                {relationships.data.incoming.map((edge) => (
                  <li key={`${edge.kind}:${edge.oracle_id}`}>
                    {KIND_LABELS[edge.kind]} from {edge.name}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <Button
            type="button"
            className="mt-4"
            disabled={setRelationships.isPending}
            onClick={() => void save()}
          >
            {setRelationships.isPending ? "Saving…" : "Save relationships"}
          </Button>
        </>
      )}
    </AdminSection>
