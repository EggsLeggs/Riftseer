...
import { DownloadIcon, FlagIcon, LayoutList, PencilLine, Rows2 } from "lucide-react";
...
import { CardPrintingsTable, OracleReferencesTable } from "@/features/cards/card-printings-table";
  ...
  return (
    <Breadcrumb>
      <BreadcrumbList>
        ...
        <BreadcrumbSeparator />
        <BreadcrumbItem>
          <BreadcrumbPage>{oracle.name}</BreadcrumbPage>
        </BreadcrumbItem>
      </BreadcrumbList>
    </Breadcrumb>
  );
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-12">
          ...
          <TableBody>
            ...
            {oracle.tags.length > 0 ? (
              <DetailRow label="Tags">
                <CardTags tags={oracle.tags} linked />
              </DetailRow>
            ) : null}
            {rulesText ? (
              <DetailRow label="Ability" alignTop>
                <CardText text={rulesText} rich={oracle.text?.rich} linkKeywords />
              </DetailRow>
            ) : null}
              ...
              <DetailRow label="Equipped" alignTop>
                <div className="space-y-1.5">
                  ...
                  {oracle.text?.equipment?.trim() ? (
                    <CardText text={oracle.text.equipment} linkKeywords />
                  ) : null}
                </div>
              </DetailRow>
            ) : null}
            ...
            <DetailRow label="Artist">
              <ArtistLink artist={printing.artist} />
            </DetailRow>
            ...
            {detail.legalities.length > 0 ? (
              <DetailRow label="Legality" alignTop>
                <CardLegalityGrid legalities={detail.legalities} className="sm:grid-cols-1" />
              </DetailRow>
            ) : null}
          </TableBody>
        </Table>
      ...
      <aside className="space-y-6 lg:col-span-4">
        ...
        <CardBuyLinks purchase={detail.purchase} printing={detail.printing} />
      </aside>
    </div>
  );
}
...
function ArtistLink({ artist }: { artist?: string }) {
  ...
  return (
    <Link
      href={searchHref(artistSearchQuery(artist))}
...
function SetLink({ detail }: { detail: OracleDetail }) {
  ...
  return (
    <Link
      href={`/sets/${set.set_code.toLowerCase()}`}
      className="hover:text-primary underline-offset-4 hover:underline"
    >
      <span className="uppercase">{set.set_code}</span>
      {set.set_name ? ` · ${set.set_name}` : null}
    </Link>
  );
}
    ...
    <div className="space-y-6">
      ...
      {detail.tokens.length > 0 ? (
        <OracleReferencesTable
          rows={detail.tokens}
          label="Tokens made"
          caption={`Tokens made by ${oracle.name}`}
        />
      ) : null}
      {detail.used_by.length > 0 ? (
        <OracleReferencesTable
          rows={detail.used_by}
          label="Used by"
          caption={`Cards that use ${oracle.name}`}
        />
      ) : null}
      {detail.characters.length > 0 ? (
        <OracleReferencesTable
          ...
          caption={`Characters linked to ${oracle.name}`}
        />
      ) : null}
      {detail.signatures.length > 0 ? (
        <OracleReferencesTable
          ...
          caption={`Signature cards linked to ${oracle.name}`}
        />
      ) : null}
    </div>
  );
    ...
    <section aria-label="Images and data">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="ghost" size="sm" asChild>
          <a href={reportCardIssueUrl(oracle, printing)} target="_blank" rel="noreferrer nofollow">
            <FlagIcon />
            Report card issue
          </a>
        </Button>
