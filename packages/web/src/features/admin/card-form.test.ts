...
const oracle: Oracle = {
  object: "oracle",
  id: "o",
  oracle_key: "warhammer",
  slug: "warhammer",
  name: "Warhammer",
  name_normalized: "warhammer",
  card_type: "Gear",
  is_token: false,
  energy: 2,
  might_bonus: 0,
...
describe("oracle editor", () => {
  ...
  test("only patches changed oracle fields", () => {
    ...
    expect(buildOraclePatch({ ...initial, tags: "Weapon, Relic" }, initial)).toEqual({
      tags: ["Weapon", "Relic"],
    });
  });
});
...
describe("printing editor", () => {
  test("keeps rarity printing-scoped", () => {
    ...
    expect(buildPrintingPatch({ ...initial, rarity: "Showcase" }, initial)).toEqual({
      rarity: "Showcase",
    });
  });
