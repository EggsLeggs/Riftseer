  ...
  return {
    ...
    client: {
      ...
      rpc: async (name: string, payload: any) => {
        ...
        if (calls.length === failRpcCall)
          return { data: null, error: { message: "constraint failed" } };
...
describe("catalogue batching", () => {
  test("keeps oracle groups whole and prunes only after every batch", async () => {
    ...
    const oracles: IngestOracle[] = Array.from(
    ...
    expect(
      calls
        .slice(0, 2)
        .every(
          (call) => call.payload.p_prune === false && call.payload.p_valid_printing_ids === null,
        ),
    ).toBe(true);
  ...
  test("never reaches prune after a deterministic batch failure", async () => {
    ...
    await expect(ingestCatalogue(client as never, [], oracles, [], [])).rejects.toThrow(
      "batch 2/2 failed",
    );
...
describe("RiftCodex quirks", () => {
  ...
  test("repairs variant flags from the printed id", () => {
    ...
    expect(printedVariantSignals("ogn-305*-298")).toMatchObject({
      signature: true,
      overnumbered: true,
    });
    expect(printedVariantSignals("ven-sp3-006")).toMatchObject({
...
describe("duplicate collapse", () => {
  test("keeps the TCGPlayer-backed upstream duplicate and backfills its image", () => {
    const result = collapseDuplicates([
      printing("z", {
        name: "Sett, Brawler",
        name_normalized: "sett brawler",
        set_code: "VEN",
        riftbound_id: "ven-004-166",
        tcgplayer_id: undefined,
        image_source_url: "https://example.com/card.png",
      }),
      printing("a", {
        name: "Sett, Brawler",
        name_normalized: "sett brawler",
...
describe("oracle grouping and relationships", () => {
  ...
  test("emits add, remove, scalar override, and clear deltas per printing", () => {
    ...
    expect(built.oracles[0]).toMatchObject({
      tags: ["Sentinel"],
      domains: ["Order"],
      energy: 2,
      text_plain: "Rules",
  ...
  test("links champions on character tags, not shared species or regions", () => {
    const poppy = oracle("poppy", {
      name: "Poppy - Paragon",
      supertype: "Champion",
      tags: ["Yordle", "Demacia", "Poppy"],
    });
    const kennen = oracle("kennen", {
      name: "Kennen - Keeper",
      supertype: "Champion",
      tags: ["Yordle", "Ionia", "Kennen"],
    });
    const legend = oracle("heart", {
    ...
    expect(edges.edges).toEqual([
      { from_oracle_key: "heart", to_oracle_key: "kennen", kind: "character" },
    ]);
  });
});
...
describe("TCGPlayer matching", () => {
    ...
    expect(
      collectorCandidates(
        printing("alt", {
          ...
          riftbound_id: "ven-113a-166",
          is_alternate_art: true,
        }),
      ),
    ).toEqual(["113a", "113"]);
    expect(
      collectorCandidates(printing("token", { collector_number: "T03", riftbound_id: "sfd-t03" })),
    ).toEqual(["t03", "03", "3"]);
  });
  ...
  test("matches base and alternate printings to their own collector products", () => {
    const base = printing("base", {
      ...
      set_code: "VEN",
      collector_number: "113",
      riftbound_id: "ven-113-166",
    });
    const alt = printing("alt", {
      ...base,
      id: "alt",
      riftcodex_id: "alt",
      riftbound_id: "ven-113a-166",
    ...
    const result = enrichPrintings(
      [base, alt],
      maps([
        product(1, "Ambessa The Wolf", "113"),
        product(2, "Ambessa The Wolf Alternate Art", "113a"),
      ]),
      new Map([["VEN", 1]]),
    );
  ...
  test("variant-distance tiebreak gives a contested product to the plain printing", () => {
    const base = printing("base", {
      name: "Vayne",
      name_normalized: "vayne",
      set_code: "VEN",
      collector_number: undefined,
      riftbound_id: undefined,
    });
    const alt = printing("alt", {
      ...base,
      id: "alt",
  ...
  test("an admin-locked product link wins contention", () => {
    const locked = printing("locked", {
      name: "Vayne",
      name_normalized: "vayne",
      set_code: "VEN",
      tcgplayer_id: "9",
      tcgplayer_id_locked: true,
    });
    const plain = printing("plain", {
      name: "Vayne",
      name_normalized: "vayne",
      set_code: "VEN",
      tcgplayer_id: "9",
...
describe("reconciliation queue", () => {
  test("queues unclaimed cards but skips sealed products and claimed links", () => {
    const productMaps = maps([
      product(1, "Unmatched Card", "12"),
      product(2, "Booster Box", null),
      product(3, "Claimed", "13"),
    ]);
  ...
  test("still files a real rarity disagreement", () => {
    const entries = buildReconciliationEntries(
      ...
      maps([product(1, "Base Card", "12", "Uncommon")]),
      new Map([["TST", 1]]),
    );
  ...
  test("upserts bounded reconciliation batches before the one final prune", async () => {
    ...
    const client = {
      rpc: async (_name: string, payload: any) => {
        calls.push(payload);
        return {
          data: { upserted: payload.p_entries.length, pruned: payload.p_prune ? 2 : 0 },
          error: null,
        };
      },
    };
    const entries = Array.from({ length: RECONCILIATION_BATCH_SIZE + 1 }, (_, index) => ({
      ...productEntryForSync(index),
    }));
    expect(await syncReconciliationQueue(client as never, entries, true)).toEqual({
      upserted: entries.length,
      pruned: 2,
    });
    expect(calls.map((call) => [call.p_entries.length, call.p_prune])).toEqual([
      [250, false],
      [1, false],
      [0, true],
    ]);
